...
def upgrade() -> None:
    ...
    content_source_type = postgresql.ENUM("youtube", "zoom", "upload", name="content_source_type", create_type=False)
    event_type = postgresql.ENUM("weekly", "monthly", "one-time", name="event_type", create_type=False)
    ...
    if "content_insights" not in inspector.get_table_names():
        op.create_table(
            "content_insights",
            sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
    ...
    if "content_relationship_targets" not in inspector.get_table_names():
        op.create_table(
